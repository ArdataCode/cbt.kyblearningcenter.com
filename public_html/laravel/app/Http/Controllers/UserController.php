...
use App\Models\UPaketSoalEssayKtg;
...
class UserController extends Controller
{
    ...
    public function paketsayadtl($idpaketdtl)
    {
        ...
        $cekpaket = Transaksi::where('fk_user_id',Auth::id())->where('status',1)->where('aktif_sampai','>=',Carbon::now())->where('fk_paket_mst',$id)->pluck('fk_paket_mst')->all();
        if(!$cekpaket){
        ...
        $paketdtl = PaketDtl::whereIn('fk_paket_mst',$cekpaket)->get();
        ...
        $idpaketdtl = PaketDtl::where('fk_paket_mst',$id)->pluck('fk_mapel_mst')->all();
        $hasilujian = UMapelMst::where('fk_user_id',Auth::user()->id)->where('is_mengerjakan',0)->whereIn('fk_mapel_mst',$idpaketdtl)->orderBy('created_at','desc')->get();
        $data_param = [
            'submenu','menu','data','paketdtl','paketvideo','paketpdf','hasilujian','paketzoom'
        ];
    ...
    public function paketgratis()
    {
        $menu = "paketsayaktg";
        $submenu="";
        $paketdtl = PaketSoalMst::where('tryout', 2)->get();
        $idpaketdtl = PaketSoalMst::where('tryout', 2)->pluck('id')->all();
        $hasilujian = UMapelMst::where('fk_user_id',Auth::user()->id)->where('is_mengerjakan',0)->whereIn('fk_mapel_mst',$idpaketdtl)->orderBy('created_at','desc')->get();
        $data_param = [
            'submenu','menu','paketdtl','hasilujian'
        ];
    ...
    public function rankingpaket($id){
        ...
        $udatapaket =UMapelMst::where('fk_mapel_mst',$id)->where('is_mengerjakan',0)->where('status',1)->orderBy('created_at','desc')->get()->unique('fk_user_id');
        $nilaitertinggi = $udatapaket->max('set_nilai'); 
        // $udatapaket = UMapelMst::where('fk_paket_soal_mst',$id)->get();
        $data_param = [
            'submenu','menu','datapaket','udatapaket','extend','nilaitertinggi'
        ];
    ...
    public function mulaiujian(Request $request,$id)
    {
        ...
        $idpaketdtl = Crypt::decrypt($request->id_paket_dtl[0]);
        $paketsoalmst = PaketSoalMst::find($idpaketsoalmst);
        
        if($paketsoalmst->tryout==1){
            $inputkode = $request->kode[0];
            if($inputkode!=$paketsoalmst->kode){
                return response()->json([
                    'status' => false,
        ...
        // Token
        // $cekpaket = Transaksi::where('fk_user_id',Auth::id())->where('status',1)->where('aktif_sampai','>=',now())->where('fk_paket_mst',$idpaketsoalmst)->get();
        // $batas = 0;
        // $unlimited = 0;
        ...
        // }
        // $cekumapel = UMapelMst::where('fk_user_id',Auth::id())->where('is_mengerjakan',0)->where('fk_paket_dtl',$idpaketdtl)->get();
        ...
        //     return response()->json([
        //         'status' => false,
        //         'message' => 'Token sudah habis, silahkan beli paket untuk mengerjakan soal lagi'
        //     ]);
        //     dd('Error');
        // }
        ...
        $data1['acak_soal'] = $paketsoalmst->acak_soal;
        ...
        if($paketsoalmst){
            ...
            if($paketsoalmst->acak_soal==1){
                $paketsoalessayktg = PaketSoalEssayKtg::where('fk_paket_soal_mst',$idpaketsoalmst)->inRandomOrder()->get();
            }else{
                $paketsoalessayktg = PaketSoalEssayKtg::where('fk_paket_soal_mst',$idpaketsoalmst)->get();
            }
        
            foreach ($paketsoalessayktg as $key) {
                $data2['fk_user_id'] = Auth::id();
                $data2['fk_u_paket_soal_mst'] = $upaketsoalmst->id;
                $ktgsoal = KategoriSoalEssay::find($key->fk_kategori_soal);
                $data2['judul'] = $ktgsoal->judul;
                $data2['ket'] = $ktgsoal->ket;
                $data2['jumlah_soal'] = $key->jumlah_soal;
                $data2['created_by'] = Auth::id();
                ...
                $data2['updated_by'] = Auth::id();
                $data2['updated_at'] = Carbon::now()->toDateTimeString(); 
                $upaketsoalktg = UPaketSoalEssayKtg::create($data2);
                
                if($paketsoalmst->acak_soal==1){
                    $paketsoaldtl = PaketSoalEssayDtl::where('fk_paket_soal_mst',$idpaketsoalmst)->where('fk_paket_soal_essay_ktg',$key->id)->inRandomOrder()->get();
                }else{
                    $paketsoaldtl = PaketSoalEssayDtl::where('fk_paket_soal_mst',$idpaketsoalmst)->where('fk_paket_soal_essay_ktg',$key->id)->get();
                }
                ...
                foreach ($paketsoaldtl as $key2) {
                    ...
                    $data3['fk_u_paket_soal_ktg'] = $upaketsoalktg->id;
                    $data3['fk_u_paket_soal_mst'] = $upaketsoalmst->id; 
                    $mastersoal = MasterSoalEssay::find($key2->fk_master_soal_essay);
                    $data3['no_soal'] = $nomor;
                    $data3['soal'] = $mastersoal->soal;
                    $data3['jawaban'] = $mastersoal->jawaban;
                    $data3['point'] = $mastersoal->point;
                    $data3['created_by'] = Auth::id();
                    $data3['created_at'] = Carbon::now()->toDateTimeString();
                    $data3['updated_by'] = Auth::id();
                    $data3['updated_at'] = Carbon::now()->toDateTimeString();
                    $upaketsoaldtl = UPaketSoalEssayDtl::create($data3);
                    $nomor++;
                }
            }
            return response()->json([
    ...
    public function updatejawabanessay(Request $request)
    {
        $upaketsoaldtl = UPaketSoalEssayDtl::find($request->idpaketdl);
        $cekismengerjakan = UMapelMst::find($upaketsoaldtl->fk_u_paket_soal_mst);
        if($cekismengerjakan->is_mengerjakan==0){
            return response()->json([
                'status' => false,
        ...
        $data['jawaban_user'] = $request->jawaban;
        $updateJawaban = UPaketSoalEssayDtl::find($request->idpaketdl)->update($data);
    ...
    public function updatejawabanessayfile(Request $request)
    {
        $upaketsoaldtl = UPaketSoalEssayDtl::find($request->idpaketdtl[0]);
        $cekismengerjakan = UMapelMst::find($upaketsoaldtl->fk_u_paket_soal_mst);
        if($cekismengerjakan->is_mengerjakan==0){
            return response()->json([
                'status' => false,
        ...
        if($request->file("jawabanessayfile")){
            if ($files = $request->file("jawabanessayfile")[0]) {
                $destinationPath = 'upload/jawabanessay/';
                $file = 'Jawaban_'.Carbon::now()->timestamp. "." .$files->getClientOriginalExtension();
                $files->move($destinationPath, $file);
                $data['jawaban_userfile'] = $destinationPath.$file;
            }
        }

        $updateJawaban = UPaketSoalEssayDtl::find($request->idpaketdtl[0])->update($data);
        
        if ($updateJawaban) {
            return response()->json([
                'status' => true,
                'message' => 'Berhasil upload file',
                'link' => asset($data['jawaban_userfile'])
            ]);
        }else{
            return response()->json([
                'status' => false,
    ...
    public function selesaiujian(Request $request)
    {
        ...
        if(count($dataupaketsoalmst->u_paket_soal_essay_dtl_r)>0){
            $data['status'] = 0;
        }else{
            $data['status'] = 1;
        }
