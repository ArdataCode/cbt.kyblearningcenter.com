...
<ol class="breadcrumb float-sm-right">
    <li class="breadcrumb-item"><a class="_kembali" href="{{url('paketsoalessayktg')}}/{{$data->fk_paket_soal_mst}}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Kembali</a></li>
</ol>
@endsection
...
<script>
  $(document).ready(function(){
    ...
    $(document).on('click', '#btn_simpan_data', function (e) {
      ...
      var url = "{{ url('storepaketsoalessaydtl') }}/{{$data->fk_paket_soal_mst}}/{{$idpaketsoalktg}}";
      $.ajaxSetup({
          headers: {
      ...
      $.ajax({
          ...
          success: function (response) {
              if (response.status == true) {
                  ...
                  reload_url(1000,"{{url('paketsoalessayktg')}}/{{$data->fk_paket_soal_mst}}");
              }else{
                Swal.fire({
                    html: response.message,
                    icon: 'error',
