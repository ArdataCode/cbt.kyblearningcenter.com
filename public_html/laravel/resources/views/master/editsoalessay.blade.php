...
<ol class="breadcrumb float-sm-right">
    <li class="breadcrumb-item"><a class="_kembali" href="{{url('mastersoalessay')}}/{{$idkategori}}"><i class="fa fa-arrow-left" aria-hidden="true"></i> Kembali</a></li>
</ol>
@endsection
              ...
              <form method="post" id="formData" class="form-horizontal">
                @csrf
                      ...
                      <div class="form-group">
                          ...
                          <textarea name="jawaban[]" id="jawaban" rows="5" class="form-control content_" placeholder="Jawaban">{!! $data->jawaban !!}</textarea>  
                      </div>  
                      <div class="form-group">
                        <label for="point">Point<span class="bintang">*</span></label>
                        <input type="number" class="form-control int" id="point" name="point[]" placeholder="Point" value="{{$data->point}}">
                      </div>
    ...
    $(document).on('click', '.btn-submit-data', function (e) {
        $('#formData').validate({
          ...
          submitHandler: function () {
            ...
            var url = "{{ url('/updatemastersoalessay') }}/{{$data->id}}";
            $.ajaxSetup({
                headers: {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == true) {
                        ...
                        reload_url(1000,"{{url('mastersoalessay')}}/{{$idkategori}}");
                    }else{
                      Swal.fire({
                          html: response.message,
                          icon: 'error',
